...
/* 	limit to a list of active concepts of which descriptions have been edited 
	this release 
*/
	drop table if exists tmp_edited_con_auth;
	create table if not exists tmp_edited_con_auth as
	select distinct a.*
...
/* list of active description of active concepts edited for this release */
	drop table if exists tmp_active_desc_auth;
	create table if not exists tmp_active_desc_auth as
	select a.*
	from curr_description_d a 
	where a.active=1 and not exists (select count(*) as total from prev_description_s b where a.id=b.id having total >= 1);
	
	alter table tmp_active_desc_auth add index idx_tmp_ad_cid(conceptid);
	alter table tmp_active_desc_auth add index idx_tmp_ad_a(active);
	alter table tmp_active_desc_auth add index idx_tmp_ad_t(term);
	
/* list of inactive description of active concepts edited for this release */
	drop table if exists tmp_inactive_desc_auth;
	create table if not exists tmp_inactive_desc_auth as
	select a.*
	from curr_description_s a
		join tmp_edited_con_auth b
			on a.conceptid = b.id
			and a.active = 0;
	
	alter table tmp_inactive_desc_auth add index idx_tmp_id_cid(conceptid);
	alter table tmp_inactive_desc_auth add index idx_tmp_id_a(active);
	alter table tmp_inactive_desc_auth add index idx_tmp_id_t(term);
...
*/ 
	...
	from tmp_active_desc_auth a
	join tmp_inactive_desc_auth b
